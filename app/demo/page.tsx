...
function DemoPageContent() {
  ...
  return (
    <div className="h-screen w-screen overflow-hidden bg-emerald-900">
      <div className="w-full h-full p-3 flex flex-col">
        {/* Header ejecutivo - más compacto */}
        <div className="flex items-center justify-between mb-2 flex-shrink-0">
          <div>
            <h1 className="text-xl font-bold text-white">Inmobiliaria Chicureo</h1>
            <p className="text-xs text-gray-400">Actualizado: {currentTime || 'Cargando...'}</p>
          </div>
          <button
            onClick={handleRefresh}
            disabled={isLoading}
            className="px-3 py-1.5 rounded-lg bg-white/20 hover:bg-white/30 backdrop-blur-sm text-white text-xs font-semibold disabled:opacity-50 transition-all flex items-center gap-1.5"
          >
        ...
        <div className="grid grid-cols-4 gap-2.5 mb-2.5 flex-1 auto-rows-fr min-h-0">
          <MetaCard
        ...
        <div className="grid grid-cols-5 gap-2.5 flex-1 auto-rows-fr min-h-0">
          <KPICard
            title="Días Firmas"
