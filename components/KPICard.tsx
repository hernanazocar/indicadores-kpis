        ...
        <div className="p-1.5">
          <div className="flex items-center justify-between mb-1">
            <div className="flex items-center gap-1">
              ...
              <p className={`text-[9px] font-bold ${colors.titleColor} uppercase tracking-wide`}>
                {title}
              </p>
            </div>
          ...
          <div className="space-y-0.5">
            ...
            >
              <div className={`${variant === 'warning' ? 'text-xl' : 'text-2xl'} font-black ${colors.valueColor}`}>
            ...
            {gapUnits !== null && variant !== 'meta' && (
              <div className="mt-1 pt-1 border-t border-slate-100">
                <div className="flex items-center justify-between mb-0">
                  <span className={`text-[9px] font-semibold ${colors.subtitleColor}`}>
                    Gap vs Meta:
                  </span>
                </div>
                <div className={`text-xs font-bold ${getGapColor()}`}>
                  {gapUnits > 0 ? '-' : '+'}{Math.abs(gapUnits).toLocaleString('es-CL')}{showPercentage ? '%' : ' unid.'}
                </div>
                {gapCLP !== null && gapCLP !== 0 && (
                  <div className={`text-[9px] font-semibold ${colors.subtitleColor} mt-0`}>
